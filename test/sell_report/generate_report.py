...
def get_summary_by_company_type_from_db(table_name, numeric_cols, year=None, month=None):
    ...
    try:
        ...
        print(f"[错误] 数据库汇总失败 ({table_name}): {str(e)}")
        import traceback
        traceback.print_exc()
        return pd.DataFrame()
...
def generate_part1_summary(pay_table, con_table, year=None, month=None):
    ...
    # 确保两个表都有必要的列
    if pay_summary_result.empty or '公司名称' not in pay_summary_result.columns:
        pay_summary_result = pd.DataFrame(columns=['公司名称', '项目类型'] + pay_cols_cn + ['_row_type'])
    if con_summary_result.empty or '公司名称' not in con_summary_result.columns:
        con_summary_result = pd.DataFrame(columns=['公司名称', '项目类型'] + con_cols_cn + ['_row_type'])
...
def generate_part2_summary(pay_table, con_table, year=None, month=None):
    ...
    # 3. 拼接两表（使用中文列名）
    # 确保两个表都有必要的列
    merge_keys = ['公司名称', '负责人', '项目类型']
    
    # 如果 pay_summary 为空或缺少列，创建一个空 DataFrame
    if pay_summary.empty or '公司名称' not in pay_summary.columns:
        pay_summary = pd.DataFrame(columns=merge_keys + pay_cols_cn)
    
    # 如果 con_summary 为空或缺少列，创建一个空 DataFrame
    if con_summary.empty or '公司名称' not in con_summary.columns:
        con_summary = pd.DataFrame(columns=merge_keys + con_cols_cn)
    ...
    # 按公司名称分组（确保列名已转换）
    if combined_result.empty or '公司名称' not in combined_result.columns:
        # 如果合并结果为空或没有公司名称列，返回空 DataFrame
        return pd.DataFrame(columns=['公司名称', '负责人', '项目类型'] + all_numeric_cols_cn + ['_row_type'])
...
def _prepare_report_data(pay_table, con_table):
    """转换列名并按公司分组数据"""
    # 转换中文列名
    pay_cn = translate_columns_to_chinese(pay_table.copy()) if pay_table is not None and not pay_table.empty else pd.DataFrame()
    con_cn = translate_columns_to_chinese(con_table.copy()) if con_table is not None and not con_table.empty else pd.DataFrame()

    def split_by_company(df):
        if df.empty or '公司名称' not in df.columns:
            return {}
        # 过滤空公司名并拆分
        valid_df = df[df['公司名称'].notna() & (df['公司名称'].astype(str).str.strip() != "")]
        return {n: valid_df[valid_df['公司名称'] == n].copy() 
                for n in valid_df['公司名称'].unique() if n and str(n).strip() != ""}

    pay_dict = split_by_company(pay_cn)
    con_dict = split_by_company(con_cn)
    all_companies = sorted(list(set(pay_dict.keys()) | set(con_dict.keys())))
    
    return pay_dict, con_dict, all_companies
def _get_output_path(year, month):
    """生成输出路径并处理已存在的文件"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    output_file = PROJECT_ROOT / "data" / f"{year}年{month}月份汇总表_{timestamp}.xlsx"
    output_file.parent.mkdir(parents=True, exist_ok=True)

    if output_file.exists():
        try:
            output_file.unlink() # 尝试删除
        except PermissionError:
            # 如果被占用，生成一个备份名
            backup_file = output_file.parent / f"{output_file.stem}_备份_{timestamp}{output_file.suffix}"
            output_file.rename(backup_file)
            return backup_file # 返回重命名后的路径
    return output_file
def _write_company_sheets(writer, companies, pay_dict, con_dict, year, month):
    """为每个公司创建独立的 Sheet 并写入数据"""
    ...
    for company in companies:
        ...
        if not df_p.empty:
            curr_row = write_dataframe_to_excel(
                writer, df_p, sheet_name, curr_row, 
                title=f"{month}月份预计回款", year=year, month=month, add_borders=True
            )
        ...
        if not df_c.empty:
            write_dataframe_to_excel(
                writer, df_c, sheet_name, curr_row, 
                title=f"{month}月份预计合同", year=year, month=month, add_borders=True
            )
...
def generate_complete_report(pay_table, con_table, year, month):
    ...
    try:
        # 1. 准备数据
        pay_dict, con_dict, all_companies = _prepare_report_data(pay_table, con_table)
        ...
        # 3. 获取输出路径
        output_file = _get_output_path(year, month)
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            if not part1_summary.empty:
                write_dataframe_to_excel(
                    writer, part1_summary, "预计汇总（总）", 0, 
                    title=f"{year}年{month}月份汇总表", merge_company=True, 
                    year=year, month=month, add_borders=True
                )
            
            # 写入详细汇总表
            if not part2_summary.empty:
                write_dataframe_to_excel(
                    writer, part2_summary, "预计汇总（详细）", 0, 
                    title=None, merge_company=True, merge_person=True, 
                    year=year, month=month, add_borders=True
                )
            
            # 写入各分公司 Sheet
            _write_company_sheets(writer, all_companies, pay_dict, con_dict, year, month)

        print(f"[成功] 报表已生成！\n路径: {output_file}")
        return str(output_file) # 返回路径方便后续 API 调用
...
class YearMonthExtractor:
    ...
    def extract(self, context: str):
        ...
        prompt = f"""
        你是一个时间提取助手。请从用户的输入中提取“年份”和“月份”。
        当前时间是：{current_year}年{current_month}月。

        规则：
        1. 如果用户说“这个月”、“本月”、“现在”，请输出当前年月：{current_year}年{current_month}月。
        2. 如果用户提到相对时间如“上个月”，请根据当前时间计算。
        3. 如果用户只说了月份没说年份，默认使用当前年份 {current_year}。
        4. 如果用户完全没提到时间，输出当前年月。
        5. 请只返回 JSON 格式，包含 "year" 和 "month" 两个键，值均为整数。

        用户输入："{context}"
        输出示例：{{"year": 2026, "month": 2}}
        """
        ...
        try:
            ...
            print(f"[AI 提取时间] 从 '{context}' 提取到: {year}年{month}月")
            return year, month
        except Exception as e:
            print(f"[提取时间失败] 错误: {e}，将使用当前时间。")
...
def generate_report_main(year=None, month=None, context=None):
    ...
    :param year: 年份（可选）
    :param month: 月份（可选）
    :param context: 自然语言上下文（可选，如果提供则优先使用 AI 提取时间）
    """
    # 1. 如果提供了 context，先用 AI 提取年月
    if context:
        extractor = YearMonthExtractor()
        year, month = extractor.extract(context)
    
    # 2. 如果既没有 context 也没有提供明确的 year/month，则处理命令行或默认值
    if year is None or month is None:
        ...
        if len(sys.argv) >= 3:
            ...
            print(f"未指定参数，使用当前日期: {year}年{month}月")
    
    # 3. 后续逻辑不变
    # 执行核心业务
    print(f">>> 准备执行报表生成任务：数据区间[{year}年{month}月]")
    
    # 1. 数据读取
    pay_table, con_table = read_detail_table(year, month)
    
    if (pay_table is None or pay_table.empty) and (con_table is None or con_table.empty):
        print(f"!!! 警告: {year}年{month}月 没有查询到任何数据记录。")
        return None
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            if not part1_summary.empty:
                write_dataframe_to_excel(writer, part1_summary, "预计汇总（总）", 0, 
                                       title=f"{year}年{month}月份汇总表", merge_company=True, 
                                       year=year, month=month, add_borders=True)
            
            if not part2_summary.empty:
                write_dataframe_to_excel(writer, part2_summary, "预计汇总（详细）", 0, 
                                       title=None, merge_company=True, merge_person=True, 
                                       year=year, month=month, add_borders=True)
            
            _write_company_sheets(writer, all_companies, pay_dict, con_dict, year, month)

        print(f"✅ 报表任务完成：{output_file.name}")
        return str(output_file)

    except Exception as e:
        print(f"❌ 报表生成流程崩溃: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
