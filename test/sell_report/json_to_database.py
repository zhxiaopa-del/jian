...
class SimpleDBManager:
    def __init__(self, config):
        ...
        # 中英文字段名映射表（延迟导入避免循环依赖）
        from config import CHINESE_TO_ENGLISH_MAPPING
        self.field_mapping = CHINESE_TO_ENGLISH_MAPPING
        self.init_db()
    ...
    def get_conn(self):
        ...
        return db_manager.get_connection(use_dict_cursor=True)
    ...
    def insert(self, category, data):
        ...
        sql = f"""
            ...
            VALUES ({placeholders})
        """
        self._execute(sql, [translated_data[k] for k in keys], show_sql=True)
        return data
    ...
    def update(self, category, data):
        """
        根据 ID 更新记录的所有字段
        :param category: 数据类别 (回款/合同)
        :param data: 从查询接口返回并经过修改的完整数据字典 (必须包含 id)
        """
        table = self.table_map.get(category)
        if not table:
            return {"code": 400, "message": f"未找到类别 {category} 对应的表"}

        record_id = data.get('id')
        exclude_fields = ["数据类别", "id"]
        update_dict = {k: v for k, v in data.items() if k not in exclude_fields}
        if not update_dict:
            return {"code": 400, "message": "更新失败：没有可更新的内容"}

        # 3. 翻译字段并构建 SQL
        translated_data = self._translate_fields(update_dict)
        
        # 构建 SET 子句: `column1` = %s, `column2` = %s ...
        set_clause = ", ".join([f"`{k}` = %s" for k in translated_data.keys()])
        
        # 构建参数列表: [值1, 值2, ..., record_id]
        params = list(translated_data.values())
        ...
        try:
            ...
            if affected_rows > 0:
                ...
                return {"message": "未找到对应的记录，数据可能已被删除"}
        except Exception as e:
            return {f"数据库更新异常: {str(e)}"}
    # 5. 删除数据
    def delete(self, category, data):
        ...
        # 转换字段名并过滤空值
        data = self._translate_fields(data)
        ...
        self._execute(sql, list(data.values()), show_sql=True)
        return data
    ...
    def select(self, category, data=None):
        ...
        # 如果提供了查询条件
        if data:
            data = self._translate_fields(data) # 自动中转英
            # 过滤空值：删除 None、空字符串、空列表等
            data = {k: v for k, v in data.items() if v is not None and v != '' and v != []}
            if data:
            ...
            with self.get_conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if show_sql and affected > 0:
                        ...
                        print(f"[成功] {msg}成功，影响了 {affected} 条记录")
                conn.commit()
                return True
...
def get_db_connection(config: Dict[str, Any] = None):
    ...
    Args:
        config: 数据库配置字典（已废弃，使用全局 db_manager）
    
    Returns:
        上下文管理器
    """
    return db_manager.get_connection(use_dict_cursor=False)
...
def get_db_manager():
    ...
    if _db_manager_instance is None:
        ...
        _db_manager_instance = SimpleDBManager(DB_CONFIG)
...
if __name__ == "__main__":
    ...
    print("查询数据")
    print("=" * 60)
    results = db.update("回款", test_data)
    print(results)
    # if results:
