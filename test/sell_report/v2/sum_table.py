...
def read_detail_table():
    """从数据库读取明细表，返回两个 DataFrame 格式的表格"""
    try:
        # 连接数据库
        conn = pymysql.connect(**DB_CONFIG)
        
        # 读取回款表数据，排除 id、创建时间、更新时间
        payment_query = """
            SELECT 负责人, 公司名称, 项目类型, 项目名称, 
                   月初预计可能回款, 月初预计确定回款, 可能回款, 确定回款, 
                   实际回款, 回款节点, 未回款金额, 未完成原因, 解决办法
            FROM payment_records
            WHERE 公司名称 IS NOT NULL AND 公司名称 != ''
        """
        payment_df = pd.read_sql(payment_query, conn)
        
   
        # 读取合同表数据，排除 id、创建时间、更新时间
        contract_query = """
            SELECT 负责人, 公司名称, 项目类型, 项目名称, 
                   月初预计可能合同, 月初预计确定合同, 可能合同, 确定合同, 
                   实际合同, 完成情况
...
def get_summary_by_company_type_from_db(table_name, numeric_cols, year=None, month=None):
    """
    从数据库按公司名称和项目类型汇总，包含小计行
    :param table_name: 表名 ('payment_records' 或 'contract_records')
    :param numeric_cols: 需要汇总的数值列列表
    :param year: 年份（可选，用于过滤数据）
    :param month: 月份（可选，用于过滤数据）
    :return: 汇总后的DataFrame，包含标记列 '_row_type'
    """
    try:
        conn = pymysql.connect(**DB_CONFIG)
        
        # 构建 WHERE 条件
        where_conditions = ["公司名称 IS NOT NULL AND 公司名称 != ''"]
        
        # 如果有日期字段，添加日期过滤（暂时不添加，因为表结构中没有日期字段）
        # 如果将来需要按日期过滤，可以添加类似：
        # if year and month:
        #     where_conditions.append(f"YEAR(创建时间) = {year} AND MONTH(创建时间) = {month}")
...
def get_summary_by_person_from_db(table_name, numeric_cols, year=None, month=None):
    ...
    try:
        ...
        query = f"""
            ...
            FROM {table_name}
            WHERE {where_clause}
            GROUP BY 公司名称, 负责人, 项目类型
            ORDER BY 公司名称, 负责人, 项目类型
        """
...
def generate_summary_by_company_type(df, numeric_cols, table_name=None, year=None, month=None):
    ...
    :param df: 原始表 (pay_table 或 con_table)，如果从数据库查询则可为 None
    ...
    # 如果提供了表名，从数据库查询
    if table_name:
        return get_summary_by_company_type_from_db(table_name, numeric_cols, year, month)
    
    # 否则使用 pandas 处理（兼容旧逻辑）
...
def process_table_by_person(df, numeric_cols, table_name=None, year=None, month=None):
    ...
    # 如果提供了表名，从数据库查询
    if table_name:
        return get_summary_by_person_from_db(table_name, numeric_cols, year, month)
...
def generate_complete_report(pay_table, con_table, year, month):
    ...
    """
    # 生成两个部分的汇总数据
    part1_summary = generate_part1_summary(pay_table, con_table)
    part2_summary = generate_part2_summary(pay_table, con_table)
    
    # 按公司拆分数据用于第三部分
    # 过滤空的公司名称
    if pay_table is not None and not pay_table.empty:
        pay_table = pay_table[pay_table['公司名称'].notna() & (pay_table['公司名称'].astype(str).str.strip() != "")]
        pay_dict = {n: pay_table[pay_table['公司名称'] == n].copy() for n in pay_table['公司名称'].unique() if n and str(n).strip() != ""}
    else:
        pay_dict = {}
    
    if con_table is not None and not con_table.empty:
        con_table = con_table[con_table['公司名称'].notna() & (con_table['公司名称'].astype(str).str.strip() != "")]
        con_dict = {n: con_table[con_table['公司名称'] == n].copy() for n in con_table['公司名称'].unique() if n and str(n).strip() != ""}
    else:
        con_dict = {}
    ...
    try:
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for company in all_companies:
                ...
                df_p = calculate_final_summaries(pay_dict.get(company), COL_CONFIG['pay'], remove_company_name=True)
                if not df_p.empty:
                ...
                df_c = calculate_final_summaries(con_dict.get(company), COL_CONFIG['con'], remove_company_name=True)
                if not df_c.empty:
                    write_dataframe_to_excel(
                        writer, 
                        df_c, 
