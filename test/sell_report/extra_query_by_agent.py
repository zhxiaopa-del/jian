...
当用户提到“修改”、“改成”、“调整”等意图时：
1. **必须提取“原有的/修改前”的金额**。
2. 在“将 A 改为 B”或“把 A 调整成 B”的句式中，**A 是你要提取的【实际回款】金额**，因为 A 是数据库中已存在的查询标记；B 是修改后的期望值，在本环节请忽略 B。
3. **严禁提取修改后的新金额**，只保留用户描述中那个“过去”的、用来定位的金额。
【必填字段（不能为空）】
- **负责人**：提取姓名。
- **公司名称**：提取公司名。
- **项目类型**：如软件开发、设备采购等。
- **项目名称**：若无明确提到，根据“公司名称+项目类型”组合生成。
【分类规则】：
